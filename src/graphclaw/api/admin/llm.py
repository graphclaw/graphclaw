...
@router.get("/providers", response_model=LLMProvidersConfig, status_code=status.HTTP_200_OK, summary="Get LLM providers")
async def get_providers(admin_user_id: AdminUserDep, storage_client: StorageClientDep) -> LLMProvidersConfig:
    data = await _load_json(_PROVIDERS_PATH, storage_client)
    return LLMProvidersConfig(**data) if data else LLMProvidersConfig()


@router.put("/providers", response_model=LLMProvidersConfig, status_code=status.HTTP_200_OK, summary="Update LLM providers")
async def put_providers(body: LLMProvidersConfig, admin_user_id: AdminUserDep, storage_client: StorageClientDep) -> LLMProvidersConfig:
    await _save_json(_PROVIDERS_PATH, storage_client, body.model_dump())
    return body


@router.post("/keys", response_model=LLMKeyResponse, status_code=status.HTTP_200_OK, summary="Store org LLM key")
async def store_llm_key(body: LLMKeyRequest, admin_user_id: AdminUserDep, secrets_client: SecretsClientDep) -> LLMKeyResponse:
...
@router.delete("/keys/{provider}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete org LLM key")
async def delete_llm_key(provider: str, admin_user_id: AdminUserDep, secrets_client: SecretsClientDep) -> None:
    try:
        await secrets_client.delete_secret(f"{_LLM_KEY_PREFIX}{provider}")
    except KeyError:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"No key for provider '{provider}'")


@router.get("/budget", response_model=BudgetConfig, status_code=status.HTTP_200_OK, summary="Get budget config")
...
@router.put("/budget", response_model=BudgetConfig, status_code=status.HTTP_200_OK, summary="Update budget config")
async def put_budget(body: BudgetConfig, admin_user_id: AdminUserDep, storage_client: StorageClientDep) -> BudgetConfig:
