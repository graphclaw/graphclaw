...
**Per-role model routing (Wave Model-Routing):** `ModelRouter` in `src/graphclaw/llm/routing.py` resolves each of six roles (`LLMRole` in `src/graphclaw/llm/roles.py`: `ORCHESTRATOR`, `SUBAGENT`, `SKILL`, `DISTILL`, `CLASSIFY`, `SUMMARIZE`) to an independently configured provider+model via `GRAPHCLAW_MODEL_<ROLE>` env vars (see `docker/.env.example`). It owns exactly one base `LLMClient` per distinct *provider* (lazily created via `create_llm_client`, cached) and vends each role a `RoleBoundLLMClient` decorator (`src/graphclaw/llm/role_client.py`) over that shared base — not one client per role, since a LiteLLM model string already carries its provider prefix. A per-agent `config.json.llm_model` override takes precedence over the `SUBAGENT` role default; a per-skill `llm_model:` frontmatter key takes precedence over the `SKILL` role default. With no `GRAPHCLAW_MODEL_*` vars set, every role falls back to `LITELLM_DEFAULT_MODEL` — fully backward compatible with a single-model deployment. Call `ModelRouter.describe()` (surfaced at `GET /health/ready` as `llm_routing`) to see what each role actually resolved to.

**Prompt/context budget (Wave Model-Routing):** `src/graphclaw/agent/prompt_budget.py` (`PromptBudget`, `PromptSection`, tool-result truncation helpers) is the single enforcement point for how much of a model's context window one turn may spend — necessary once `LLMRole.ORCHESTRATOR`/`SUBAGENT` can point at a small local model (e.g. 32k-token Ollama models) rather than always a large hosted window. `ContextConfig` in `src/graphclaw/config.py` holds every `GRAPHCLAW_CONTEXT_*` knob. Per-request state (the ACL `CallerContext`, session id, and an active `ToolSetRegistry`) is isolated via `contextvars` in `src/graphclaw/agent/turn_state.py`, since `MainOrchestrator` is a single process-wide singleton shared by every concurrent user.
