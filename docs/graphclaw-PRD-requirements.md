...
2. Browser redirected to IdP authorization endpoint:
     GET https://accounts.google.com/o/oauth2/v2/auth
       ...
       &redirect_uri=https://app.workgraph.io/auth/callback
       &response_type=code
       &scope=openid email profile
       &state=[CSRF-token]                 <- prevents open redirect
...
4. IdP redirects back with authorization code:
     GET https://app.workgraph.io/auth/callback?code=[code]&state=[token]

5. API server validates:
   - state matches CSRF token stored in session (prevents CSRF)
   - redirect_uri matches registration (hardcoded, not user-controlled)

6. API server exchanges code for IdP tokens:
     POST https://oauth2.googleapis.com/token
       client_id, client_secret (from Secrets Manager),
       code, redirect_uri, grant_type=authorization_code,
       code_verifier (PKCE)

7. API server validates IdP ID token (signed JWT from Google/Microsoft/GitHub):
   - Signature valid against IdP public keys (fetched from JWKS endpoint)
   - Issuer matches expected IdP
   - Audience matches platform client_id
   - Token not expired
   - nonce matches (replay protection)

8. API server resolves or creates UserNode:
   - email from ID token -> lookup in Postgres
   - New user: provision UserNode, agent container, S3 prefix, IAM role
   - Existing user: update last_login_at, refresh cached org memberships

9. API server issues platform JWT:
     {
       "sub":     "USER-john-doe",
       "email":   "john@acme.com",
       "org_ids": ["ORG-work", "ORG-personal"],
       "roles":   { "ORG-work": "ADMIN", "ORG-personal": "MEMBER" },
       "iat":     1741267200,
       "exp":     1741268100,           <- 15-minute expiry
       "jti":     "[uuid]"              <- unique ID for revocation
     }
     Signed with RS256 using RSA private key from Secrets Manager
...
Access JWT:
  Expiry:     15 minutes
  Algorithm:  RS256 (asymmetric — private key stays in Secrets Manager)
  Storage:    httpOnly cookie (not localStorage — blocks XSS theft)
  Flags:      Secure (HTTPS only), SameSite=Strict (blocks CSRF)

Refresh token:
  Expiry:     30 days rolling
  Format:     Opaque random string (256 bits, not a JWT)
  DB storage: SHA-256 hash in Postgres refresh_tokens table
  Rotation:   New token issued on every use, old token invalidated

Token refresh (transparent to user):
  Browser detects JWT expiry approaching (at 13-minute mark)
  POST /auth/refresh with refresh token cookie
  API server validates refresh token hash against Postgres
  Issues new access JWT + new refresh token (rotation)
  Old refresh token immediately invalidated

Logout:
  POST /auth/logout
  API server adds JWT jti to Redis revocation set (TTL = remaining JWT lifetime)
  API server deletes refresh token record from Postgres
